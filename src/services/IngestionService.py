import asyncio
from pathlib import Path
from typing import Any

from sqlalchemy.ext.asyncio import AsyncSession

from src.chunkers import SemanticChunker
from src.models import AssetModel, ChunkModel
from src.parsers import SectionBuilder, UnstructuredPDFParser
from src.schemas.ingestion import SemanticChunk
from src.stores.llm.LLMInterface import LLMInterface
from src.stores.vectordb import (
    VectorDBInterface,
    VectorDocument,
)


class IngestionService:
    """Coordinate PDF parsing, semantic chunking, persistence, and indexing."""

    def __init__(
        self,
        parser: UnstructuredPDFParser,
        section_builder: SectionBuilder,
        semantic_chunker: SemanticChunker,
        embedding_provider: LLMInterface,
        vector_db: VectorDBInterface,
    ) -> None:
        self._parser = parser
        self._section_builder = section_builder
        self._semantic_chunker = semantic_chunker
        self._embedding_provider = embedding_provider
        self._vector_db = vector_db

    async def ingest_asset(
        self,
        session: AsyncSession,
        asset_id: int,
    ) -> list[SemanticChunk]:
        """Process one existing Asset and return its public chunk output."""
        if asset_id <= 0:
            raise ValueError("asset_id must be greater than zero")

        asset = await AssetModel.get_by_id(
            session=session,
            asset_id=asset_id,
        )
        if asset is None:
            raise ValueError(f"Asset was not found: {asset_id}")

        pdf_path = Path(asset.file_path).expanduser().resolve()
        if not pdf_path.is_file():
            raise FileNotFoundError(f"Asset PDF was not found: {pdf_path}")
        if pdf_path.suffix.lower() != ".pdf":
            raise ValueError(f"Asset file must be a PDF: {pdf_path}")

        await AssetModel.update_status(
            session=session,
            asset_id=asset_id,
            status="PROCESSING",
        )

        vector_ids: list[str] = []

        try:
            # Unstructured parsing is synchronous and CPU/IO intensive.
            parsed_elements = await asyncio.to_thread(
                self._parser.parse,
                pdf_path,
            )

            sections = self._section_builder.build(
                document_name=asset.document_name,
                elements=parsed_elements,
            )
            if not sections:
                raise ValueError("No document sections were generated")

            semantic_chunks = await self._semantic_chunker.chunk(sections)
            if not semantic_chunks:
                raise ValueError("No semantic chunks were generated")

            # Keep database changes uncommitted until vector indexing succeeds.
            await ChunkModel.replace_asset_chunks(
                session=session,
                asset_id=asset_id,
                chunks=semantic_chunks,
                metadata={
                    "parser": self._parser.__class__.__name__,
                    "chunker": self._semantic_chunker.__class__.__name__,
                },
                commit=False,
            )

            embeddings = await self._embedding_provider.embed_documents(
                [chunk.text for chunk in semantic_chunks]
            )
            if len(embeddings) != len(semantic_chunks):
                raise RuntimeError(
                    "Embedding count does not match semantic chunk count"
                )

            vector_documents = []
            for chunk, embedding in zip(semantic_chunks, embeddings):
                vector_id = self._build_vector_id(asset_id, chunk.chunk_id)
                vector_ids.append(vector_id)

                vector_documents.append(
                    VectorDocument(
                        id=vector_id,
                        text=chunk.text,
                        embedding=embedding,
                        metadata={
                            "asset_id": asset_id,
                            "project_id": asset.project_id,
                            "chunk_id": chunk.chunk_id,
                            "document_name": chunk.document_name,
                            "section_title": chunk.section_title,
                            "page_number": chunk.page_number,
                        },
                    )
                )

            await self._vector_db.initialize()

            # Re-indexing replaces all vectors that belong to this asset.
            await self._vector_db.delete_by_asset_id(asset_id)
            await self._vector_db.upsert(vector_documents)

            total_pages = max(
                element.page_number for element in parsed_elements
            )
            await AssetModel.mark_completed(
                session=session,
                asset_id=asset_id,
                total_pages=total_pages,
                total_chunks=len(semantic_chunks),
                commit=False,
            )

            await session.commit()
            return semantic_chunks

        except Exception as exc:
            await session.rollback()

            # Remove partially indexed vectors from this failed ingestion run.
            try:
                if vector_ids:
                    await self._vector_db.delete_by_ids(vector_ids)
            except Exception:
                pass

            await AssetModel.mark_failed(
                session=session,
                asset_id=asset_id,
                error_message=str(exc),
            )
            raise

    async def get_asset_output(
        self,
        session: AsyncSession,
        asset_id: int,
    ) -> list[dict[str, str | int]]:
        """Return stored chunks using the exact public output contract."""
        records = await ChunkModel.get_by_asset(
            session=session,
            asset_id=asset_id,
        )
        return [record.to_output_dict() for record in records]

    async def remove_asset_index(
        self,
        session: AsyncSession,
        asset_id: int,
    ) -> int:
        """Remove stored chunks and vectors for an asset."""
        if asset_id <= 0:
            raise ValueError("asset_id must be greater than zero")

        await self._vector_db.initialize()
        deleted_vectors = await self._vector_db.delete_by_asset_id(asset_id)
        await ChunkModel.delete_by_asset(
            session=session,
            asset_id=asset_id,
        )
        return deleted_vectors

    async def close(self) -> None:
        """Release external provider resources."""
        await self._embedding_provider.close()
        await self._vector_db.close()

    @staticmethod
    def _build_vector_id(asset_id: int, chunk_id: str) -> str:
        return f"asset_{asset_id}_{chunk_id}"
